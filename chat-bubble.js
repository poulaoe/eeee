  ...
  async function loadMini() {
    ...
    try {
      ...
      if (msgs.length > lastCount && lastCount > 0) {
        badge.textContent = "+" + (msgs.length - lastCount);
      ...
      container.innerHTML = msgs
        ...
        .map(function (m) {
          ...
          return (
            ...
            (own ? "rgba(52,152,219,.1)" : "rgba(255,255,255,.06)") +
            ";border-radius:6px;padding:7px;border-left:2px solid " +
            (own ? "#3498db" : "#27ae60") +
            ';margin-bottom:4px"><div style="font-weight:700;color:#a8d8ff;font-size:.72rem;margin-bottom:3px">' +
            (own ? "👤 Toi" : m.user) +
            ' <span style="color:#555">' +
            t +
            '</span></div><div style="color:#d4e4f7">' +
            String(m.message || "")
              .replace(/&/g, "&amp;")
              .replace(/</g, "&lt;") +
            "</div></div>"
