...
const IS_ANAT_SUBJECT = () =>
  String(SUBJECT_NAME || "").toLowerCase() === "anat";
...
function readWindowNameStorage() {
  if (
    ...
    !window.name.startsWith(windowNameStoragePrefix)
  )
    return {};
  try {
    const parsed = JSON.parse(
      window.name.slice(windowNameStoragePrefix.length),
    );
...
function showFatalInitError(error) {
  ...
  const stack =
    error && error.stack
      ? `<pre style="white-space:pre-wrap;margin-top:10px;color:#ffd2d2">${error.stack}</pre>`
      : "";
...
function getSignedUser() {
  return (safeGetItem("qcm_signed_user") || "").trim();
}
...
function sanitizeQuestionEntry(entry) {
  ...
  const parsedAnswerIndex = Number.isInteger(source.a)
    ? source.a
    : parseInt(source.a, 10);
  const safeAnswerIndex = Number.isFinite(parsedAnswerIndex)
    ? parsedAnswerIndex
    : 0;
  ...
  let nextAnswerIndex = uniqueOptions.findIndex(
    (option) => normalizeOptionKey(option) === normalizeOptionKey(correctText),
  );
...
function syncChapterFilterSelection(chapters) {
  ...
  Array.from(filter.options).forEach((option) => {
    option.selected =
      option.value === "all" ? wanted.size === 0 : wanted.has(option.value);
  });
}

function getCurrentGradingConfig() {
  const gradingMode =
    document.getElementById("partiel-grading")?.value || "bac";
  ...
  if (gradingMode === "custom") {
    ...
    const good = parseCustomValue(
      document.getElementById("custom-good")?.value,
      0.25,
    );
    const bad = parseCustomValue(
      document.getElementById("custom-bad")?.value,
      0.083,
    );
...
function styleOptionsPanel() {
  ...
  const sandboxOptions =
    document.getElementById("sandbox-options") ||
    document.getElementById("module-sandbox");
  const commonOptions =
    document.getElementById("common-options") ||
    document.getElementById("module-partiel");
  if (sandboxOptions) {
...
function saveResultForCurrentUser(result) {
  ...
  const history = Array.isArray(accounts[currentUser].history)
    ? accounts[currentUser].history
    : [];
...
function renderFeedbackPanel() {
  ...
  if (reports.length === 0) {
    html +=
      '<div style="color:#888;font-size:.85rem">Aucun signalement pour le moment.</div>';
  } else {
...
async function loadQuizChatMessages() {
  ...
  try {
    ...
    if (
      !window.QCM_REMOTE ||
      typeof window.QCM_REMOTE.fetchChatMessages !== "function"
    ) {
    ...
    if (!Array.isArray(messages)) {
      container.innerHTML =
        '<div style="color:#666">Erreur chargement messages</div>';
      return;
    }
    if (messages.length === 0) {
      container.innerHTML =
        '<div style="color:#888;text-align:center;padding:20px">Aucun message. Sois le premier !</div>';
      return;
    }
    ...
    if (status)
      status.textContent =
        "Erreur chat: " + (error && error.message ? error.message : "reseau");
  }
}
...
async function sendQuizChatMessage() {
  ...
  if (
    !window.QCM_REMOTE ||
    typeof window.QCM_REMOTE.pushChatMessage !== "function"
  ) {
  ...
  try {
    ...
    document.getElementById("quiz-chat-status").textContent =
...
function initSandboxPage() {
  ...
  if (!messageZone) {
    ...
    if (homeCard)
      homeCard.insertBefore(
        messageZone,
        accountZone || homeCard.querySelector(".btn-start") || null,
      );
  }
  ...
  if (
    ...
    throw new Error(
      "Structure HTML incomplète: un ou plusieurs éléments requis sont absents.",
    );
  }
  ...
  chapterCount.textContent = Array.from(
    new Set(questionBank.map((q) => q.ch)),
  ).length;
  ...
  messageZone.textContent = questionBank.length
    ? ""
    : "Aucune question disponible pour cette matière.";
  ...
  if (gradingSelect && gradingCustom) {
    const refreshGradingVisibility = () => {
      gradingCustom.style.display =
        gradingSelect.value === "custom" ? "block" : "none";
    };
...
function populateChapterOptions() {
  ...
  Array.from(new Set(questionBank.map((q) => q.ch)))
...
function renderChapterList() {
  ...
  if (!preview) {
    ...
    modulePanel.insertBefore(
      preview,
      modulePanel.querySelector(".module-hint"),
    );
  }
  preview.innerHTML = "";
  const chapters = Array.from(new Set(questionBank.map((q) => q.ch))).sort(
    (a, b) => a.localeCompare(b, "fr"),
  );
  if (chapters.length === 0) {
    preview.innerHTML =
      '<div style="color:#aaa;font-size:.9rem">Pas de chapitres disponibles.</div>';
    return;
  }
  chapters.forEach((ch) => {
    ...
    chip.onclick = () => {
      ...
      quickList
        ...
        .forEach((el) =>
          el.classList.toggle(
            "active",
            getSelectedQuickChapter().includes(el.textContent),
          ),
        );
      preview.querySelectorAll("[data-preview-chapter]").forEach((block) => {
        ...
        block.style.display =
          selected.length === 0 || selected.includes(chapter)
            ? "block"
            : "none";
      });
    };
...
function ensurePartielCustomQuestionInput() {
  ...
  if (
    !partielPanel ||
    !sizeSelect ||
    document.getElementById("partiel-size-custom")
  )
    return;
...
function switchTab(id) {
  switchModule(id === "tab-chap" ? "chapitres" : id.replace("tab-", ""));
}
...
function syncExamSizeSelect(
  ...
  const candidate =
    Number.isFinite(previous) && previous > 0 ? previous : preferredDefault;
  const values = Array.from(
    new Set(
      (presetValues || []).filter(
        (v) => Number.isFinite(v) && v > 0 && v <= maxCount,
      ),
    ),
  );
  ...
  const fallback = values.includes(preferredDefault)
    ? preferredDefault
...
function refreshExamSizeFromFilters() {
  ...
  const prepared = prepareQuestionPool(BANK);
  const questionBank = prepared.questions;
  const selectedChapters = getSelectedQuickChapter().length
    ? getSelectedQuickChapter()
    : getSelectedChapters(filter);
  const pool = selectedChapters.length
    ? questionBank.filter((q) => selectedChapters.includes(q.ch))
    : questionBank;
  syncExamSizeSelect(
    "exam-size",
    pool.length,
...
function loadRecentQuestionKeys(subjectName) {
  const key =
    RECENT_QUESTIONS_STORAGE_PREFIX +
    String(subjectName || SUBJECT_NAME || "General");
  try {
...
function pickSessionWithHistory(pool, totalCount, options) {
  ...
  if (
    config.forceDifferent &&
    lastDrawQuestionKeys.length &&
    candidatePool.length > wanted
  ) {
    ...
    if (fresh.length >= wanted) candidatePool = fresh;
      ...
      const older = shuffle(
        candidatePool.filter((q) => previousSet.has(questionKey(q))),
      );
      candidatePool = fresh.concat(older.slice(0, wanted - fresh.length));
    }
  }
...
function getBalancedDraw(pool, totalCount) {
  ...
  const chapters = Array.from(byChapter.entries()).map(
    ([chapter, questions]) => ({
      chapter,
...
function attachTimerToHeader() {
  ...
  if (!timer) {
    ...
    timer.id = "timer";
    timer.style.cssText =
      "font-weight:800;font-size:1rem;color:#fff;padding:6px 12px;background:#0f2f52;border:1px solid #29507a;border-radius:8px;margin-left:auto";
...
function startExam(options) {
  ...
  const sandboxSelectedChapters = quickSelected.length
    ? quickSelected
    : getSelectedChapters(filter);
  const sandboxPool =
    sandboxSelectedChapters.length === 0
      ? questionBank
      : questionBank.filter((q) => sandboxSelectedChapters.includes(q.ch));

  let pool = sandboxPool;
  let requestedCount = parseInt(
    document.getElementById("exam-size")?.value || "25",
    10,
  );
  let timerMins = 0;
  ...
  if (activeModule === "partiel") {
    ...
    const presetCount = parseInt(
      document.getElementById("partiel-size")?.value || "25",
      10,
    );
    const customCount = parseInt(
      document.getElementById("partiel-size-custom")?.value || "",
      10,
    );
  ...
  const timerValue =
    document.getElementById("global-timer")?.value ||
    document.getElementById("partiel-timer")?.value;
  ...
  if (pool.length === 0) {
    alert(
      "Aucune question disponible pour les chapitres sélectionnés. Choisis d'autres chapitres.",
    );
    return;
  }
  ...
  if (!currentSession.length) {
    alert(
      "Impossible de générer un questionnaire. Réessaie avec d'autres options.",
    );
    return;
  }
  if (pool.length < requestedCount)
    alert(
      `Les chapitres choisis contiennent seulement ${pool.length} questions. Le quiz utilisera toutes les questions disponibles.`,
    );

  if (prepared.duplicateQuestionCount || prepared.duplicateOptionCount) {
    const details = [];
    if (prepared.duplicateQuestionCount)
      details.push(
        `${prepared.duplicateQuestionCount} question(s) en double ignorée(s)`,
      );
    if (prepared.duplicateOptionCount)
      details.push(
        `${prepared.duplicateOptionCount} réponse(s) en double retirée(s)`,
      );
    setMessageZone(details.join(" • "));
  } else if (config.forceDifferent) {
    setMessageZone(
      "Nouveau tirage généré avec une préférence pour des questions différentes.",
    );
  } else {
    setMessageZone("");
  }
...
function showResults() {
  ...
  (Array.isArray(currentSession) ? currentSession : []).forEach((q, i) => {
    ...
    const ut =
      ua !== null && ua !== undefined && ua >= 0 && ua < opts.length
        ? opts[ua]
        : "—";
  ...
  const sessionSize = Math.max(
    1,
    Array.isArray(currentSession) ? currentSession.length : 1,
  );
  ...
  try {
    recordLeaderboardResult({
      ...
      subject: SUBJECT_NAME,
      user: currentUser || "Invité",
      score: (finalNoteValue / 2).toFixed(2),
      max: 10,
      pct,
    });
  } catch (e) {
  ...
  const qcHTML = (Array.isArray(currentQC) ? currentQC : [])
    .map((qc, i) => {
      const item = qc || {};
  ...
  (function saveErrors() {
    try {
      ...
      (Array.isArray(currentSession) ? currentSession : []).forEach(
        function (q, i) {
          var ua = answers[i],
            ca = shuffledCorrects[i];
          if (ua !== ca) {
            var key = (q.ch || "") + "::" + (q.q || "").slice(0, 80);
            var exists = qcmErrors.findIndex(function (e) {
              return e.key === key;
            });
            ...
            if (exists >= 0)
              qcmErrors[exists].count = (qcmErrors[exists].count || 1) + 1;
      ...
      localStorage.setItem(
        "qcm_erreurs",
        JSON.stringify(qcmErrors.slice(0, 200)),
      );
    } catch (e) {}
  })();
}
...
(function initChatBubble() {
  ...
  async function loadMiniChatMessages() {
    ...
    try {
      ...
      if (messages.length === 0) {
        container.innerHTML =
          '<div style="color:#888;text-align:center;padding:10px">Aucun message</div>';
        return;
      }
      const user =
        (typeof currentUser !== "undefined" && currentUser) ||
        localStorage.getItem("qcm_signed_user") ||
        "Invité";
      container.innerHTML = messages
        .slice(0, 15)
        .map((msg) => {
          ...
          <div class="chat-mini-msg-text">${String(msg.message || "")
            .replace(/&/g, "&amp;")
            .replace(/</g, "&lt;")
            .replace(/>/g, "&gt;")}</div>
        </div>`;
      ...
      console.warn("Mini chat error:", e);
    }
  }
